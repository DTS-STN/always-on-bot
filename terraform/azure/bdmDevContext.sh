#!/bin/bash
export ARM_CLIENT_ID=$(az keyvault secret show --vault-name bdmDevDPSKeyVault --name bdm-dev-decd-ei-callback-bot-appid --query value -otsv)
export ARM_CLIENT_SECRET=$(az keyvault secret show --vault-name bdmDevDPSKeyVault --name bdm-dev-decd-ei-callback-bot-password--query value -otsv)
